...
void StringTree::insert(const string& word, int count, STNode* node) {  // Добавление элемента
    ...
    if (res < 0) {  // Наше слово больше чем в узле
        if (node->right != nullptr) insert(word, count, node->right);  // Если правая ветка сущ. то идем рекурсивно направо
            ...
            node->right = new STNode{ word, count, node };
        ...
        if (node->left != nullptr) insert(word, count, node->left);  // Если левая ветка сущ. то идем рекурсивно налево
            ...
            node->left = new STNode{ word, count, node };
            if (word.size() > length) length = word.size() / 2 + 1;
        }
    }
...
int StringTree::remove(const string& word, STNode* node, int side, bool isHead) {
    ...
    int res = node->word.compare(word);  // Если будет < 0, значит слово, переданное В функцию, больше того, что в узле
    if (res < 0) {  // Наше слово больше чем в узле
        return remove(word, node->right, 1, false);  // Рекурсивно идем направо
    }
    else if (res > 0) {  // Наше слово меньше чем в узле
        return remove(word, node->left, -1, false);  // Рекурсивно идем налево
    }
        ...
        if (node->right == nullptr && node->left == nullptr) {  // Если листьев/ветвей нет
            ...
            else side > 0 ? node->parent->right = nullptr : node->parent->left = nullptr;  // подтираем ссылку на удаляемый элемент
            delete node;
        }
            ...
            *node = *(node->right != nullptr ? node->right : node->left);  // копируем ее в удаляемую ноду
            ...
            hardRemove(node->left, node);  // Используем специальную рекурсивную функцию удаления
        }
        return count;
    }
}

void StringTree::hardRemove(STNode* node, STNode* rmnode) {
    if (node->right != nullptr) hardRemove(node->right, rmnode);  // Идем к самому правому элементу
    else {
        ...
        if (node->left != nullptr) {  // Если есть ответвления влево, то переносим левую часть повыше
            ...
            *node = *leftNode;
            ...
            node->parent->right == node ? node->parent->right = nullptr : node->parent->left = nullptr;
            delete node;
        }
    }
}
...
int StringTree::findWord(const string& word, STNode* node, int steps, bool isHead)
{
    if (isHead) node = head;
    if (node == nullptr) cout << "Узлов пройдено: " << steps << "\n";
    else {
        int res = node->word.compare(word);
        if (res < 0) return findWord(word, node->right, steps + 1, false);
        else if (res > 0) return findWord(word, node->left, steps + 1, false);
        else {
            cout << "Узлов пройдено: " << steps << "\n";
            return node->count;
        }
    }
    return 0;
